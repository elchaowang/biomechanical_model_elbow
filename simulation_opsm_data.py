...
muscle_group = {'TRIlong': Muscle('TRIlong', PCSA=15.93, max_iso_force=771.8 * 0.9, LM_opti=0.14843214266132004),
                'BIClong': Muscle('BIClong', PCSA=5.28, max_iso_force=525.1 * 0.9, LM_opti=0.12741665324312956),
                'BRA': Muscle('BRA', 6.18, max_iso_force=1177.37 * 0.9, LM_opti=0.09465961482578092),
                'BRD': Muscle('BRD', 1.7, max_iso_force=276.0 * 0.9, LM_opti=0.18844965719547496),
                'PRO': Muscle('PRO', 4.11, max_iso_force=557.2 * 0.9, LM_opti=0.05360455206381705)}
...
def start_simulation(PATH, testNum, cost):
    ...
    prediction_tosave.to_excel(PATH + 'test' + str(testNum) + 'result_with_Fpe_cost_01_modified_muscledata.xlsx')


if __name__ == '__main__':

    PATH = './IsometricData/FCR/test'
    print(PATH)
    cost_f = 1
    computing_pool = list()
    testNums = [1, 2, 3, 4, 5]
