...
def start_simulation(PATH, testNum):
    ...
    :return:
    '''
    recons_df = pd.read_excel(PATH + 'recons.xlsx', skiprows=range(0, 1))
    # print(recons_df)
    frame_num = recons_df['Frame']
    elbow_flex_ = recons_df['elbow flexion']
    ...
    for i in range(len(frame_num)):
        ...
        opti_activation, joint_torque = static_opti(tmp_frame, sub, muscle_group=muscle_group, frameNum=i, init_opti=init_act)
    ...
    prediction_tosave.to_excel(PATH + 'test' + str(testNum) + 'result_without_Fpe_06.xlsx')


PATH = './IsometricData/YY/test'
